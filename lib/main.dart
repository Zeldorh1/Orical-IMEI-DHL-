...
class _SetupFlowState extends State<SetupFlow> {
  ...
  void _goVerified() => setState(() => _step = 2);

  Future<void> _goPasskey(String pass) async {
  ...
  Widget build(BuildContext context) {
    ...
    switch (_step) {
      case 0:
        return EntryStep(initial: _fields, onNext: _goEntry);
      case 1:
        return VerifyStep(fields: _fields, onConfirmed: _goVerified, onBack: () => setState(() => _step = 0));
      case 2:
        return PasskeyStep(onNext: _goPasskey, onBack: () => setState(() => _step = 1));
    }
...
class _EntryStepState extends State<EntryStep> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Step 1 of 3: Enter device info')),
      body: Form(
        key: _formKey,
        child: ListView(
  ...
  Widget _digits(TextEditingController c, String label, int max, {bool required = false, bool luhn = false, bool digitsOnly = true}) {
      ...
      body: ListView(
        ...
        children: [
          Container(
            ...
            child: const Text(
              'On this same phone, dial *#06# (or open Settings → About) and confirm every '
              'value below matches the value the device itself reports. '
              'Only confirm if every value is identical.',
          ...
          CheckboxListTile(
            ...
            title: const Text('I have visually compared every value above against this device\'s own *#06# / About output, and every value matches.'),
          ),
          const SizedBox(height: 16),
          Row(
            children: [
    ...
    return Scaffold(
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            const Text(
              'Set a passkey that will be required to edit or unlock device info. '
              'Choose something only IT staff knows. A one-time recovery code will be shown next.',
        ...
        children: [
          TextField(
            ...
            decoration: InputDecoration(
              labelText: 'Passkey or recovery code',
              errorText: _error,
            ),
...
class AdminPage extends StatelessWidget {
  ...
  Future<void> _factoryReset(BuildContext context) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
          'This will erase the stored device info, admin passkey, and recovery code. '
  ...
  Future<void> _reverify(BuildContext context) async {
    ...
    await Navigator.of(context).push(MaterialPageRoute(
      builder: (_) => ReverifyPage(fields: fields, onConfirmed: () async {
        await Store.write(Keys.lastVerifiedMs, DateTime.now().millisecondsSinceEpoch.toString());
        onChanged();
      }),
    ));
      ...
      body: ListView(
        children: [
          ListTile(
            ...
            subtitle: const Text('Re-attest that stored values match the device\'s real *#06#'),
            onTap: () => _reverify(context),
          ),
          ListTile(
            leading: const Icon(Icons.edit_outlined),
            title: const Text('Edit device info'),
            subtitle: const Text('Update IMEI, IMEI2, EID, MEID, Serial, Model'),
            onTap: () async {
              await Navigator.of(context).push(MaterialPageRoute(
          ...
          ListTile(
            ...
            onTap: () => Navigator.of(context).push(MaterialPageRoute(
              builder: (_) => const ChangePassPage(),
            )),
          ),
          const Divider(),
          ListTile(
...
class ReverifyPage extends StatefulWidget {
  const ReverifyPage({super.key, required this.fields, required this.onConfirmed});
  final DeviceFields fields;
  final Future<void> Function() onConfirmed;
  @override
  State<ReverifyPage> createState() => _ReverifyPageState();
}
...
class _ReverifyPageState extends State<ReverifyPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Re-verify')),
      body: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          const Text(
            'Dial *#06# (or open Settings → About) on this device and compare every value below. '
            'Only confirm if every value still matches.',
            style: TextStyle(color: Colors.black54),
          ),
          ...
          CheckboxListTile(
            ...
            onChanged: (v) => setState(() => _attested = v ?? false),
            controlAffinity: ListTileControlAffinity.leading,
            title: const Text('I have just visually compared every value against the device\'s own *#06# output, and every value still matches.'),
          ),
          const SizedBox(height: 16),
          FilledButton(
            onPressed: _attested
            ...
            child: const Padding(
              padding: EdgeInsets.symmetric(vertical: 12),
              child: Text('Stamp as verified now'),
            ),
          ),
        ],
      ),
...
class _EditPageState extends State<EditPage> {
  ...
  Future<void> _save() async {
    ...
    final confirmed = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (_) => Scaffold(
          appBar: AppBar(title: const Text('Verify updated values')),
          body: VerifyStep(
            fields: fields,
